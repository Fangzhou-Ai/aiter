...
one buffer resource, then folds the expert offset into that offset, wraps as soon as
an expert's byte offset passes 2**32 -- it silently reads a *different* expert's
weights. E=896 with model_dim=3584 and inter_dim=3072 -- one rank holding every
expert -- makes w1 9.2 GB and w2 4.6 GB, so both stages cross the line.

**Why this test cannot be a torch-reference test.** The reference has to dequantize
fp4 weights to f32 (8x the bytes), and in this parametrisation ``w1 = 2 * w2``
always holds (w1 is ``[E, 2*inter, model]``, w2 is ``[E, model, inter]``). So
``w2 > 4 GB`` forces ``w1 > 8 GB`` and a >68 GB reference: OOM, always. That is
exactly why the bug survived -- the existing harnesses structurally cannot reach
these shapes.

**Calibrated twin instead.** Give one distinct weight code to a LOW expert (whose
byte offset never wraps) and the SAME code to a HIGH expert (whose offset does),
background code everywhere else. Route every token to one expert at a time. Then:

  * addressing correct -> out(high) == out(low), bit for bit; the inputs are equal
  * addressing wraps    -> out(high) collapses toward the background level

No reference is needed, and the assertion is exact rather than a tolerance. This
also side-steps stage1's activation nonlinearity: outputs are only ever compared
against outputs that *should* be identical, never converted back to a code value.
"""
...
def test_stage2_past_4gib(a_dtype, inter_dim):
    ...
    if a_dtype == "bf16":
        a2 = torch.full(
            (TOKEN, TOPK, inter_dim), 1.0, dtype=DTYPE, device="cuda"
        )
        a2_scale = None
    else:
