...
def _gemm2_body(
    ...
    *,
    ...
    # Raw-pointer B loads once B outgrows what a buffer resource can address; see
    # the comment at the _bq_expert_base construction below. No cache-name tag is
    # needed: this is a pure function of NE/N_OUT/K, and the kernel name already
    # carries all three, so the two variants cannot collide in the JIT cache.
    ...
    # per-expert 64-bit re-base: b_load_s_base below is an i32 soffset that used to
    # carry `e * N_OUT * _K_HALF`, which overflows i32 for a large expert count:
    # NE=896 with N_OUT=3584 and _K_HALF=1536 reaches 4.9e9 for the last expert.
    # Fold the expert into the descriptor's 64-bit base and keep the soffset
    # expert-relative.
    ...
    # The raw-pointer (GEP) B path only earns its keep when the offsets can
    # actually overflow; buffer_load keeps hardware bounds checking and a cheaper
    # address encoding, so stay on it whenever B fits. Threshold is 2 GB, not 4:
    # intermediate signed index math breaks before the unsigned voffset does.
    # Either way the expert base is added in 64-bit, which is what fixes the bug;
    # the per-lane offsets below stay expert-relative and small.
        ...
        for j in range_constexpr(4):
            for half in range_constexpr(2):
                if const_expr(kt * 2 + half >= _n_real_half):
                    continue
                if const_expr(_use_bqptr64):
                    # bq_base_ptr is already at this expert, so the byte offset
                    # here is expert-relative and cannot overflow.
