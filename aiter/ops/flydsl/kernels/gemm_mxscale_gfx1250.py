...
from aiter.ops.flydsl.kernels.quant_utils import (
    emit_cvt_scalef32_pk8_fp8_bf16,
    emit_f32_to_e2m1,
    emit_mx_e8m0_scale,
)
        ...
        def _emit_tile(
            ...
            def _emit_stage1_quant_blocks(entries):
                ...
                for m_off, mx_block, chunks in entries:
                    ...
                    _e8m0_dtype = (
                        _MxDtype.FP8_E4M3 if _q_is_fp8 else _MxDtype.FP4_E2M1
                    )
                    ...
                    block_scale_f32 = (e8m0 << c23_i32).bitcast(T.f32)

                    payload_writes = []
                    if const_expr(_q_is_fp8):
                        # Pack each chunk's 8 cols via the native gfx1250
                        # v_cvt_scalef32_pk8_fp8_bf16 (8 bf16 -> 8 fp8 e4m3
                        # bytes as v2i32), matching the standalone quant
                        # kernel bit-for-bit.
                        for out_col_base, vals in chunks:
                            ...
                            bf16_vals = [
                                arith.trunc_f(T.bf16, v) for v in vals
                            ]
                            bf16x8 = vector.from_elements(
                                T.vec(8, T.bf16), bf16_vals
                            )
                            store_val = emit_cvt_scalef32_pk8_fp8_bf16(
                                bf16x8, block_scale_f32,
